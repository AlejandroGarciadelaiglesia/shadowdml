...
void swd::blacklist::scan(const swd::request_ptr& request) const {
    ...
    for (const auto& parameter: parameters) {
        each_path = parameter->get_path();
        each_value = parameter->get_value();
        ...
        *******/
        ...
        if ( (each_path.find("GET") != std::string::npos) ) {
            ...
            if ( !(ServerResponseGet.strBody.find("LEGAL") != std::string::npos) ){
                ...
                parameter->set_value(parameter->get_value() + "  Attack type: " + ServerResponseGet.strBody);
            }

        }else if ( (each_path.find("POST") != std::string::npos) ) {
            // POST request
            ...
            if ( !(ServerResponsePost.strBody.find("LEGAL") != std::string::npos) ){
                parameter->set_threat(true);
                parameter->set_critical_blacklist_impact(true);
                parameter->set_value(parameter->get_value() + "  Attack type: " + ServerResponsePost.strBody);
            }
        } 
